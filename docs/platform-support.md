...
With a target configured, the supervisor verifies that the selected route uses this VPN, binds the probe to its assigned IP and requires a successful TCP handshake before showing connected. It repeats this service check about every 15 seconds. Starting with 1.1.2, an isolated service failure changes the UI to verification in progress while the supervisor retries; three consecutive failures disconnect the session. A success resets the failure counter. This avoids tearing down the VPN on a single lost SYN without falsely showing connected during verification. A TCP handshake establishes reachability, not database authentication or query correctness. Without a target, connected means the adapter, address, routes and MTU have passed validation. These service probes currently apply only to Windows.
